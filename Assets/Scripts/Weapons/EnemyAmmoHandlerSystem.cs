        ...
        protected override void OnUpdate()
        {
            ...
            Entities.WithNone<Pause>().WithAll<EnemyComponent>().ForEach(
                ...
                ) =>
                {
                    ...
                    switch (enemyWeapon)
                    {
                        case { IsFiring: 1, Duration: 0, firingStage: FiringStage.None }:
                        case { IsFiring: 1, Duration: 0, } when
                            animatorWeightsComponent.aimWeight <= enemyWeapon.animTriggerWeight:
                            enemyWeapon.firingStage = FiringStage.Start;
                            break;
                        case { IsFiring: 1, Duration: 0, firingStage: FiringStage.Update }
                            when
                            animatorWeightsComponent.aimWeight > enemyWeapon.animTriggerWeight:
                        {
