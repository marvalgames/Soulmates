...
[System.Serializable]
public class SandboxPostProcessPass : ScriptableRenderPass
{
    ...
    private SandboxBloomEffectComponent m_BloomEffect;
    ...
    public SandboxPostProcessPass(Material bloomMaterial, Material compositeMaterial)
    {
        m_BloomMaterial = bloomMaterial;
        m_compositeMaterial = compositeMaterial;
        //m_globalTexture = globalTexture;
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        //Debug.Log("EXECUTE PASS");
        VolumeStack stack = VolumeManager.instance.stack;
        m_BloomEffect = stack.GetComponent<SandboxBloomEffectComponent>();

        CommandBuffer cmd = CommandBufferPool.Get();

        using (new ProfilingScope(cmd, new ProfilingSampler("Sandbox Post Process Effects")))
        {
    ...
    private void SetupBloom(CommandBuffer cmd, RTHandle source)
    {
        ...
        // Setup bloom on uber
        var tint = m_BloomEffect.tint.value.linear;
        ...
        var bloomParams = new Vector4(m_BloomEffect.intensity.value, tint.r, tint.g, tint.b);
        bloomMaterial.SetVector("_Bloom_Params", bloomParams);
        var useRGBM = false;
        bloomMaterial.SetFloat("_Bloom_RGBM", useRGBM ? 1f : 0f);
        
        

        //cmd.SetGlobalTexture("_Bloom_Texture", m_globalTexture);
        cmd.SetGlobalTexture("_Bloom_Texture", m_BloomMipUp[0]);
        cmd.SetGlobalFloat("_BloomIntensity", m_BloomEffect.intensity.value);
        //Debug.Log("INTENSITY " + m_BloomEffect.intensity.value);
    }
