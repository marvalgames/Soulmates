...
public class ScreenSpaceAmbientOcclusionSettings
{
    ...
    [SerializeField] internal float DirectLightingStrength = -2;
    [SerializeField] internal float Radius = 4;
    ...
    [SerializeField] internal float Falloff = 90f;
...
public class SandboxScreenSpaceAmbientOcclusionFeature : ScriptableRendererFeature
{
    ...
    public override void Create()
    {
        ...
        {
            m_customPass = new SandboxScreenSpaceAmbientOcclusion();
        }
    ...
    public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
    {
        ...
        //{
        bool shouldAdd = m_customPass.Setup(ref m_Settings, ref renderer, ref m_Material, ref m_BlueNoise256Textures);
        if (shouldAdd)
        {
    ...
    //        m_customPass.SetTarget(renderer.cameraColorTargetHandle,
    //            renderer.cameraDepthTargetHandle, renderer, m_Material);
    //    }
    //}
...
public class SandboxScreenSpaceAmbientOcclusion : ScriptableRenderPass
{
    ...
    public SandboxScreenSpaceAmbientOcclusion()
    {
    ...
    public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
    {
        ...
        RenderingUtils.ReAllocateIfNeeded(ref m_SSAOTextures[0], m_AOPassDescriptor, FilterMode.Bilinear,
            TextureWrapMode.Clamp, name: "_SSAO_OcclusionTexture0");
        RenderingUtils.ReAllocateIfNeeded(ref m_SSAOTextures[1], m_AOPassDescriptor, FilterMode.Bilinear,
            TextureWrapMode.Clamp, name: "_SSAO_OcclusionTexture1");
        RenderingUtils.ReAllocateIfNeeded(ref m_SSAOTextures[2], m_AOPassDescriptor, FilterMode.Bilinear,
            TextureWrapMode.Clamp, name: "_SSAO_OcclusionTexture2");
        ...
        RenderingUtils.ReAllocateIfNeeded(ref m_SSAOTextures[k_FinalTexID], m_AOPassDescriptor, FilterMode.Bilinear,
            TextureWrapMode.Clamp, name: "_SSAO_OcclusionTexture");

        // Configure targets and clear color
        ConfigureTarget(m_CurrentSettings.AfterOpaque
            ? m_Renderer.cameraColorTargetHandle
            : m_SSAOTextures[k_FinalTexID]);
    ...
    public void SetTarget(RTHandle cameraColorTargetHandle,
        RTHandle cameraDepthTargetHandle, ScriptableRenderer scriptableRenderer, Material material)
    {
        m_Material = material;
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        using (new ProfilingScope(cmd, new ProfilingSampler("Sandbox SSAO")))
        {
            ...
            GetPassOrder(m_BlurType, m_CurrentSettings.AfterOpaque, out int[] textureIndices,
                out ShaderPasses[] shaderPasses);

            //RenderAndSetBaseMap(ref cmd, ref renderingData, ref m_Renderer, ref m_Material,
            // ref m_CameraDepthTarget, ref m_SSAOTextures[0], ShaderPasses.AmbientOcclusion);


            // Execute the SSAO
            RTHandle cameraDepthTargetHandle = m_Renderer.cameraDepthTargetHandle;
            RenderAndSetBaseMap(ref cmd, ref renderingData, ref m_Renderer, ref m_Material,
                ref cameraDepthTargetHandle, ref m_SSAOTextures[0], ShaderPasses.AmbientOcclusion);
            ...
            for (int i = 0; i < shaderPasses.Length; i++)
            {
                ...
                RenderAndSetBaseMap(ref cmd, ref renderingData, ref m_Renderer, ref m_Material,
                    ref m_SSAOTextures[baseMapIndex], ref m_SSAOTextures[targetIndex], shaderPasses[i]);
            }
    ...
    public bool Setup(ref ScreenSpaceAmbientOcclusionSettings featureSettings, ref ScriptableRenderer renderer,
        ref Material material, ref Texture2D[] blueNoiseTextures)
    {
        ...
        //else
        {
            ...
            renderPassEvent = m_CurrentSettings.AfterOpaque
    ...
    private static void RenderAndSetBaseMap(ref CommandBuffer cmd, ref RenderingData renderingData,
        ref ScriptableRenderer renderer, ref Material mat, ref RTHandle baseMap, ref RTHandle target,
        ShaderPasses pass)
    {
        if (IsAfterOpaquePass(ref pass))
        {
    
            Blitter.BlitCameraTexture(cmd, baseMap, renderer.cameraColorTargetHandle,
                RenderBufferLoadAction.Load, RenderBufferStoreAction.Store, mat, (int)pass);
        }
    ...
    // private void SetupSSAO(CommandBuffer cmd, RTHandle source)
    // {
    // }
    private static void GetPassOrder(BlurTypes blurType, bool isAfterOpaque, out int[] textureIndices,
        out ShaderPasses[] shaderPasses)
    {
        switch (blurType)
        {
