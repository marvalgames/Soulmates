...
namespace FIMSpace.Generating
{
    public abstract partial class SpawnRuleBase
    {
        ...
        /// <summary> Helper value which can be switched by right click on the node header for individual node </summary>
        [System.NonSerialized] public bool DebugV = false;
            ...
            if (GUILayout.Button(new GUIContent(foldout + head + foldout, tip), FGUI_Resources.HeaderStyle))
            {
                ...
                if (AllowDuplicate())
                    if (FGenerators.IsRightMouseButton())
                    {
                        ...
                        menu.AddItem( new GUIContent( "Open Node Script File" ), false, () =>
                        {
                            MonoScript script = MonoScript.FromScriptableObject( this );
                            AssetDatabase.OpenAsset( script );
                        } );
                        ...
                        menu.AddItem( new GUIContent( "Switch Debug Variable" ), DebugV, () => { DebugV = !DebugV; } );
